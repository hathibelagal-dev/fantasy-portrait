...
from ..models.wan_video_dit import flash_attention,attention
...
class SingleStreamBlockProcessor(nn.Module):
    ...
    def __call__(
        ...
        if len(adapter_proj.shape) == 4:
            ...
            ip_key = self.ip_adapter_single_stream_k_proj(adapter_proj).view(b*latents_num_frames, -1, n, d)
            ip_value = self.ip_adapter_single_stream_v_proj(adapter_proj).view(b*latents_num_frames, -1, n, d)
            adapter_x = attention(adapter_q, ip_key, ip_value, attn_mask=adapter_attn_mask)
...
class PortraitAdapter(nn.Module):
    ...
    def init_proj(self, cross_attention_dim=5120):
        proj_model = MultiProjModel(
            adapter_in_dim=self.adapter_in_dim,
    ...
    def set_portrait_adapter(self,wan_dit):
        ...
        for name in wan_dit.attn_processors.keys():
            ...
            attn_procs[name] = SingleStreamBlockProcessor(
                context_dim=self.adapter_proj_dim,
    ...
    def get_adapter_proj(self, adapter_fea=None):
        split_sizes = [6, 6, 30, 512]
        headpose, eye, emo, mouth = torch.split(adapter_fea, split_sizes, dim=-1) # B frames c
    ...
    def split_audio_adapter_sequence(self, adapter_proj_length, num_frames=80):
        ...
        pos_idx_range = [[idx-int(half_tokens_pre_latents_frame),idx+int(half_tokens_pre_latents_frame)] for idx in pos_idx]
        pos_idx_range[0] = [-(int(half_tokens_pre_latents_frame)*2-pos_idx_range[1][0]),pos_idx_range[1][0]]
        return pos_idx_range

    def split_tensor_with_padding(self, input_tensor, pos_idx_range, expand_length=0):
        pos_idx_range = [[idx[0]-expand_length,idx[1]+expand_length] for idx in pos_idx_range]
        sub_sequences = []
            ...
            for progress_id, timestep in enumerate(progress_bar_cmd(self.scheduler.timesteps)):
                ...
                if ip_cfg_scale and ip_cfg_scale > 1.0:
                    ...
                    if cfg_scale != 1.0: 
                        ...
                        noise_pred = noise_pred_no_cond + cfg_scale * (noise_pred_noaudio - noise_pred_no_cond) + ip_cfg_scale * (noise_pred_posi - noise_pred_noaudio) 
                    else:
                        noise_pred = noise_pred_noaudio + ip_cfg_scale * (noise_pred_posi - noise_pred_noaudio)
