...
from utils import extract_faces_from_video,change_video_fps,draw_rects_on_frames
...
def compute_max_xy(rect_list):
    ...
    return min(max_left_move,0.25),min(max_right_move,0.25),min(max_up_move,0.25),min(max_down_move,0.25)
...
def create_mask(image,bounding_boxes,proj_split,video_rect_list):
    ...
    for i,(face_rect,video_rect) in enumerate(zip(bounding_boxes,video_rect_list)):
        max_left_move,max_right_move,max_up_move,max_down_move = compute_max_xy(video_rect)
...
def process_multi_video(portrait_model,face_aligner,pd_fpg_motion,image,video_list):
    ...
    width, height = image.size
    bounding_boxes, _, score = face_aligner.face_alignment_module.face_detector.detect(np.array(image)[:, :, ::-1])
    num_faces = len(bounding_boxes)
    ...
    if len(video_list) != num_faces:
        raise ValueError(f"the length of video_list {len(video_list)} is not equal to num_faces {num_faces}!")
    ...
    with torch.no_grad():
        for video_path in video_list:
            emo_feat_all, head_emo_feat_all, fps, num_frames, rect_list, frame_list = get_emo_feature(video_path, face_aligner, pd_fpg_motion)
    ...
    for face_motion_feat_ in face_motion_feat:
        ...
        proj_split_, adapter_context_lens = portrait_model.split_tensor_with_padding(adapter_proj, pos_idx_range, expand_length=0)
        proj_split.append(proj_split_)

    proj_split = torch.cat(proj_split[::-1],dim=-2)

    adapter_attn_mask,extend_bounding_boxes = create_mask(image,bounding_boxes,proj_split,video_rect_list)

    return proj_split,adapter_attn_mask,extend_bounding_boxes,frame_list_list,num_frames,video_list

def main(args):
    now = datetime.now()
    timestamp_str = now.strftime("%Y%m%d_%H%M%S")
    image_name = os.path.splitext(os.path.basename(args.input_image_path))[0]
    video_name = "_".join(os.path.splitext(os.path.basename(video_path))[0] for video_path in args.driven_video_path)
    args.output_path = os.path.join(args.output_path, f"{timestamp_str}_{image_name}_{video_name}") 
    ...
    for video_path in args.driven_video_path:
        output_driven_video_path = os.path.join(driven_video_dir, os.path.basename(video_path))
    ...
    if len(args.driven_video_path) == 1:
        ...
        args.driven_video_path = [os.path.join(tmp_dir,p) for p in os.listdir(tmp_dir) if p.endswith(".mp4")]

    proj_split,adapter_attn_mask,extend_bounding_boxes,frame_list_list,num_frames,video_list_sample = process_multi_video(portrait_model, face_aligner, pd_fpg_motion,image, args.driven_video_path)
    ...
    save_video(video_audio, os.path.join(args.output_path, f"output.mp4"), fps=args.fps, quality=5)
